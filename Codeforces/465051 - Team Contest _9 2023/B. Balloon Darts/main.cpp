...
template <typename T, typename V> int cmp(T a, V b) {
  return (a -= b) < -EPS ? -1 : (a > EPS ? 1 : 0);
}
template <typename T, typename V> bool iseq(T a, V b) { return cmp(a, b) == 0; }
template <typename T> bool iseq0(T a) { return cmp(a, 0) == 0; }
template <typename T, typename V> bool islte(T a, V b) {
  return cmp(a, b) != 1;
}
template <typename T, typename V> bool isgte(T a, V b) {
  return cmp(a, b) != -1;
}
template <typename T, typename V> bool islt(T a, V b) {
  return cmp(a, b) == -1;
}
template <typename T, typename V> bool isgt(T a, V b) { return cmp(a, b) == 1; }
template <typename T> int sign(T val) { return cmp(val, 0); }
...
int ccw(const pt &a, pt b, pt c) {
  if (a == b)
    return (a == c ? 0 : +3); // same point or different
  b = b - a, c = c - a;
  if (sign(b.cross(c)) == +1)
...
struct Line : public Segment {
  ...
  bool operator==(const Line &l) const {
    return iseq0((a - b).cross(l.a - l.b));
  };
};
...
struct Ray : public Segment {
  ...
  bool operator==(const Ray &r) const {
    return a == r.a && slope(a, b, false) == slope(r.a, r.b, false);
  };
};
...
bool parallel(const Line &a, const Line &b) {
  return (a.b - a.a).cross(b.b - b.a) == 0;
}
bool orthogonal(const Line &a, const Line &b) {
  return (a.a - a.b).dot(b.a - b.b) == 0;
}
...
bool intersect(const pt &p, const Line &l) {
...
bool intersect(const Segment &s, const Ray &r) {
  auto d1 = (s.a - s.b).cross(r.b - r.a), d2 = (s.a - r.a).cross(r.b - r.a),
       d3 = (s.a - s.b).cross(s.a - r.a);
  if (abs(d1) <= EPS)
    return r.a.cross(r.b, s.a) == 0 &&
           (r.a.dot(r.b, s.a) >= 0 || r.a.dot(r.b, s.b) >= 0); // NOT BACK
  return sign(d1) * sign(d2) >= 0 && sign(d1) * sign(d3) >= 0 &&
         abs(d2) <= abs(d1);
}
...
struct Circle {
  ...
  Circle(pt a, pt b, pt c) {
    ...
    bool valid = intersection(Line(ABmid, ABmid + ABnorm),
                              Line(BCmid, BCmid + BCnorm), this->c);
    assert(valid); // unless at least two points are identical
    r = (a - this->c).len();
  }

  friend bool intersect(const pt &p, const Circle &c) {
    return islte((p - c.c).norm(), c.r * c.r);
  }
...
int point_in_polygon(const pt &p, const vector<pt> &polygon) {
  ...
  for (int i = 0, j = 1; i < n; i++, j++, j %= n) {
    if (ccw(polygon[j], polygon[i], p) == 0)
      return 0;
...
int ray_and_polygon(const Ray &r, const Polygon &polygon) {
  ...
  for (int i = 0, j = 1, k = 2; i < n; i++, j++, k++, j %= n, k %= n) {
    if (!intersect(Segment(polygon.vert[i], polygon.vert[j]), r))
      continue;
    ...
    if (x == 0)
      ...
      return 1; // Entering from an edge
  }
  return ans;
}
...
vector<pt> convexhull(vector<pt> &p, bool strict = false) {
  ...
  for (int i = 0; i < n; ch[k++] = p[i++]) // lower hull
    while (k >= 2 &&
           sign((ch[k - 1] - ch[k - 2]).cross(p[i] - ch[k - 1])) <= sgn)
      --k;
...
struct PointInConvex {
  ...
  int check(pt point) {
    ...
    if (intersect(point, Segment(pt(0, 0), seq[0])))
      return 0;
    if (seq.size() <= 2)
      return -1;
    ...
    if (ok == -1)
      return -1;
    if (intersect(point, Segment(seq[l], seq[l + 1])))
      return 0;
    return 1;
  }
};
...
struct Welzl {
  ...
  vector<pt> go(int i = 0, vector<pt> cir = {}) {
    if (cir.size() == 3 || i == (int)points.size())
      return cir;
...
int32_t main() {
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = i + 1; j < 4; j++) {
      debug(i, j);
      // choose a line (point i, point j)
      // proceed with the line and get more 3 balloons outside
      vector<pt> three;
      int k = 0;
      for (; three.size() < 3 && k < n; k++) {
        if (k == i || k == j)
          continue;
        if (abs(ccw(points[i], points[j], points[k])) == 1) {
          three.push_back(points[k]);
        }
      }
      ...
      for (int I = 0; I < 3; I++) {
        for (int J = I + 1; J < 3; J++) {
          k = backup_k;
          debug(I, J);
          // collect more two points outside the two lines
          vector<pt> two;
          int x = 0;
          while (x == I || x == J)
            x++;
          if (abs(ccw(points[i], points[j], three[x])) == 1 &&
              abs(ccw(three[I], three[J], three[x])) == 1)
            two.push_back(three[x]);
          for (; two.size() < 2 && k < n; k++) {
            if (abs(ccw(points[i], points[j], points[k])) == 1 &&
                abs(ccw(three[I], three[J], points[k])) == 1) {
              two.push_back(points[k]);
            }
          }

          debug(points[i], points[j]);
          debug(three[I], three[J]);
          debug(two);
          ...
          for (; k < n; k++) {
            if (abs(ccw(points[i], points[j], points[k])) == 1 &&
                abs(ccw(three[I], three[J], points[k])) == 1 &&
                abs(ccw(two.front(), two.back(), points[k])) == 1) {
              ok = false;
            }
          }
