...
// Codeforces -> Educational Codeforces Round 1 -> F. Cut Length
...
int32_t main() {
    ...
    Polygon poly(n);
    for (int i = 0; i < n; i++) {
        cin >> poly.verts[i], poly.verts[i] = poly.verts[i] * 1000;
    }
    ...
    for (int i = 0; i < m; i++) {
        ...
        vector<pt> inter = intersection(poly, line);
        double ans = 0;

        for (int i = 0; i + 1 < inter.size(); i += 2) {
            ans += (inter[i] - inter[i + 1]).len();
        }
