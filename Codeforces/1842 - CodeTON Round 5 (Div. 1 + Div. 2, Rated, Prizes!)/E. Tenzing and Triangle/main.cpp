...
int32_t main() {
    ...
    for (int i = 0; i < n; i++) {
        int x, y, c;
        cin >> x >> y >> c;
        points[x].push_back({y, c});
        ans += c;
    }

    Segtree sg(k + 1);
    sg.update(0, k, Update(0, Update::forced));
    sg.update(0, 0, Update(A, Update::forced));
    for (int x = k - 1; ~x; x--) {
        int i = k - x;

        for (auto [y, c]: points[x]) {
            sg.update(0, y, Update(-c, Update::relative));
        }

        sg.update(i, i, Update(sg.query(0, i).mn, Update::forced));
        sg.update(i + 1, k, sg.query(i).mn);
        if (i < k) sg.update(0, i, Update(A, Update::relative));
    }

    cout << ans + sg.query(k, k).mn << '\n';
