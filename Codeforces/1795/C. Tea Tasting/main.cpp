...
void solve() {
    ...
    ll tea[n], tasters[n], extra[n];
    memset(extra, 0, sizeof extra);
    for (int i = 0; i < n; i++) cin >> tea[i];
    for (int i = 0; i < n; i++) cin >> tasters[i], i > 0 && (tasters[i] += tasters[i - 1]);

    segtree<ll> sg(n + 1, 0, [](ll l, ll r) { return l + r; });

    for (int i = 0; i < n; i++) {
        int j = lower_bound(tasters + i, tasters + n, tea[i] + (i > 0 ? tasters[i - 1] : 0)) - tasters;
        if (j > i)
            sg.relative_update(i, j - 1, 1);
        if (j < n)
            extra[j] += tea[i] - ((j > 0 ? tasters[j - 1] : 0) - (i > 0 ? tasters[i - 1] : 0));
    }
    ...
    for (int i = 0; i < n; i++) {
        debug(sg.query(i, i));
        cout << extra[i] + (ll)sg.query(i, i) * (tasters[i] - (i > 0 ? tasters[i - 1] : 0)) << " ";
    }
    cout << endl;
}
