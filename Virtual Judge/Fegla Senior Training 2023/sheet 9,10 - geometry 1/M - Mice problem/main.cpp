...
int32_t main() {
    ...
    Polygon rect(4);
    cin >> rect.vert[0] >> rect.vert[2];
    rect.vert[1].x = rect.vert[0].x;
    rect.vert[1].y = rect.vert[2].y;
    rect.vert[3].x = rect.vert[2].x;
    rect.vert[3].y = rect.vert[0].y;
    ...
    double time[4];
    auto get_interval = [&](int i) -> pair<double, double> {
        int cnt = 0;

        if (vol[i] == pt(0, 0)) {
            if (point_in_polygon(ini[i], rect.vert) == 1) {
                return {0, inf};
            } else {
                return {inf, -inf};
            }
        ...
        Ray ray(ini[i], ini[i] + vol[i]);

        if (ray_and_polygon(ray, rect) != 1) {
            return {inf, -inf};
        }

        pt inter;
        for (int j = 1; j <= 4; j++) {
            if (intersection(Segment(rect.vert[j % 4], rect.vert[j - 1]), ray, inter)) {
                time[cnt++] = (inter - ini[i]).len() / vol[i].len();
            }
        }
        ...
        assert(1 <= cnt && cnt <= 2);
        if (cnt == 1) return {0, time[0]};
        return {time[0], time[1]};
    };

    for (int i = 0; i < n; i++) {
        auto [l, r] = get_interval(i);
        debug(l, r);
        mn = max(mn, l);
        mx = min(mx, r);
    }
