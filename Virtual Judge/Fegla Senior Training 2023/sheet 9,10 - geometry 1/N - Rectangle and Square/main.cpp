...
struct Segment {
    ...
    bool operator==(const Segment &s) { return a == s.a ? b == s.b : a == s.b && b == s.a; };
...
struct Line {
    ...
    bool operator==(const Line &l) { return iseq0(a.cross(l.a, l.b)); };
...
bool is_square(vector<pt> &points) {
    // choose two points with the first one
    for (int i = 1; i < 4; i++) {
        for (int j = i + 1, k; j < 4; j++) {
            for (k = 1; k < 4; k++)
                if (k != i && k != j) break;
            set<int> norms;
            norms.insert((points[i] - points[0]).norm());
            norms.insert((points[j] - points[0]).norm());
            norms.insert((points[i] - points[k]).norm());
            norms.insert((points[j] - points[k]).norm());
            if (points[k].dot(points[i], points[j]) == 0 &&
                points[0].dot(points[i], points[j]) == 0 && norms.size() <= 1) {
                return true;
            }
        }
    }
...
bool is_rect(vector<pt> &points) {
    ...
    for (int i = 1; i < 4; i++) {
        for (int j = i + 1, k; j < 4; j++) {
            ...
            norms.insert((points[j] - points[k]).norm());
            if (points[k].dot(points[i], points[j]) == 0 &&
                points[0].dot(points[i], points[j]) == 0 && norms.size() <= 2) {
                return true;
            }
        }
    }
...
int32_t main() {
    ...
    for (int i = 0; i < (1 << 8); i++) {
        ...
        vector<pt> square, rect;
        for (int j = 0; j < 8; j++) {
            if (i >> j & 1)
                square.push_back(points[j]);
            else
                rect.push_back(points[j]);
        }

        assert(square.size() == 4);
        assert(rect.size() == 4);

        if (is_square(square) && is_rect(rect)) {
            cout << "YES\n";
            vector<int> square, rect;
            for (int j = 0; j < 8; j++) {
                if (i >> j & 1)
                    square.push_back(j + 1);
                else
                    rect.push_back(j + 1);
            }
            for (int i = 0; i < 4; i++)
