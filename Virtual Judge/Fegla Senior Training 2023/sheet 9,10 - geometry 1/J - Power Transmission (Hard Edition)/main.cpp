...
struct Wire {
    ...
    int c;
    Wire() {}
    Wire(pt slope, int c) : slope(slope), c(c) {}
    array<int, 3> toarr() const { return {slope.x, slope.y, c}; }
    bool operator<(const Wire &w) const { return toarr() < w.toarr(); }
};
...
int32_t main() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
            pt slope = poles[i] - poles[j];
            int s = slope.x < 0 ? -1 : 1;
            int g = gcd(abs(slope.x), abs(slope.y));
            slope.x *= s, slope.y *= s; // make x always positive
            slope.x /= g, slope.y /= g; // this doesn't affect the slope
            if (slope.x == 0) {
                slope.y = abs(slope.y);
            }
            int c = slope.x * poles[i].y - slope.y * poles[i].x;
            int d = slope.x * poles[i].y - slope.y * poles[i].x;
            assert(c == d);
            auto wire = Wire(slope, c);
            if (!wires.count(wire)) {
                wires.insert(wire);
            }
        }
