...
int32_t main() {
    ...
    vector<int> order(n);
    iota(all(order), 0);
    sort(all(order), [&](int l, int r) { return ang[l] < ang[r]; });
    ...
    long double mx = 0;
    for (int i = 1; i <= n; i++) {
        int j = order[i % n], k = order[i - 1];
        if (ang[j] - ang[k] < 0)
            mx = max(mx, ang[j] - ang[k] + 2 * PI);
        else
            mx = max(mx, ang[j] - ang[k]);
    }
    ...
    cout << fixed << setprecision(10) << (2 * PI - mx) * 180 / PI << '\n';
