...
int dfs(int i) {
    if (i == n) {
        int ans = 0;
        for (int j = 0; j < n; j++) {
            if (point_in_polygon(points[j].first, poly) == OUT) continue;
            ans += points[j].second;
        }
        return ans;
    }
    ...
    if (poly.size() <= 1 || (points[i].first - poly.end()[-1]).cross(poly.end()[-2] - poly.end()[-1]) > 0) {
        poly.push_back(points[i].first);
        maxit(ans, dfs(i + 1));
        poly.pop_back();
    }
    return ans;
...
int32_t main(int32_t argc, char **argv) {
    ...
    sort(points, points + n, [](auto &a, auto &b) {
        return a.first.cross(b.first) > 0;
    });

    if (n == 1) {
        cout << max(0ll, points[0].second) << endl;
        return 0;
    }
