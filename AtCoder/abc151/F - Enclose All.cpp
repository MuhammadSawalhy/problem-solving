...
int point_in_polygon(pt p, vector<pt> &polygon) {
    ...
    for (int i = 1; i < n; i++) {
        if (ccw(p, polygon[i], polygon[i - 1]) == 0) return polygon.pop_back(), 0;
...
struct Welzl {
    ...
    vector<pt> go(int i = 0, vector<pt> cir = {}) {
        auto p = pair<int, int>(points[i].x, points[i].y);
...
int32_t main() {
    ...
    for (int i = 0, x, y; i < n; i++)
        cin >> x >> y, points.push_back(pt(x, y));

    auto c = Welzl(points).get_circle();
    cout << fixed << setprecision(17) << c.r << endl;
