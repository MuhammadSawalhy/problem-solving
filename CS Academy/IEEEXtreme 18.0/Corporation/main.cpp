...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        int x;
        cin >> x;
        a.update(i, i, Update(x, Update::forced));
    }
    ...
    while (q--) {
        ...
        if (t == 0) {
            ...
            cin >> c;
            for (int i = l; i <= r; i++) {
                int v = a.query(i, i).sum;
                if (c > v) {
                    b.update(i, i, Update(1, Update::relative));
                } else if (c < v) {
                    b.update(i, i, Update(-1, Update::relative));
                }
            }
            a.update(l, r, Update(c, Update::forced));
        } else if (t == 1) {
            int c;
            cin >> c;
            if (c > 0)
                b.update(l, r, Update(1, Update::relative));
            else if (c < 0)
                b.update(l, r, Update(-1, Update::relative));
            a.update(l, r, Update(c, Update::relative));
        } else if (t == 2) {
            int x = a.query(l, r).sum;
            int y = r - l + 1;
            debug(x, y);
            int g = gcd(abs(x), abs(y));
            cout << x / g << '/' << y / g << endl;
        } else if (t == 3) {
            int x = b.query(l, r).sum;
            int y = r - l + 1;
            int g = gcd(abs(x), abs(y));
            cout << x / g << '/' << y / g << endl;
        }
