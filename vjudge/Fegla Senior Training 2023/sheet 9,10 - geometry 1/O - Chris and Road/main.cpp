...
int32_t main() {
    ...
    // for (int i = 0; i < n; i++) {
    //     bus[i] = pt(7000 + 500 * cos(2 * PI / n * i), 500 + 500 * sin(2 * PI / n * i));
    // }

    for (auto &p: bus)
        cin >> p;

    // we need the bottom right vertex of the bus
    // we also need to check each edge and if it hits the pedestrain or not

    bool hits = ray_and_polygon(Ray(pt(0, 0), pt(v, u)), bus) == 1;
    ...
    int right_bottom = 0;
    for (int i = 1; i < n; i++) {
        if (bus[i].x > bus[right_bottom].x || (bus[i].x == bus[right_bottom].x && bus[i].y < bus[right_bottom].y)) {
            right_bottom = i;
        }
    }
    ...
    int bottom_right = 0;
    for (int i = 1; i < n; i++) {
        if (bus[i].y < bus[bottom_right].y || (bus[i].y == bus[bottom_right].y && bus[i].x > bus[bottom_right].x)) {
            bottom_right = i;
        }
    }
    ...
    double x = 0;
    double t = 0;
    if (bottom_right > right_bottom) right_bottom += n;
    for (int i = bottom_right; i <= right_bottom; i++) {
        debug(bus[i % n].x, x);
        // assert(bus[i % n].x > x);
        if (bus[i % n].x == x) continue;
        double dy = min(1.0 * u / v, (bus[i % n].y - y) / (bus[i % n].x - x)) * (bus[i % n].x - x);
        t += (bus[i % n].x - x) / v;
        y += dy;
        x = bus[i % n].x;
    }
