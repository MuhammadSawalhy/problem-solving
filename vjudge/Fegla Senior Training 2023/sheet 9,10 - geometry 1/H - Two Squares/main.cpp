...
int32_t main() {
    ...
    vector<vector<pt>> square(2, vector<pt>(5));
    ...
    for (int i = 1; i <= 4; i++) {
        for (int j = 1; j <= 4; j++) {
            if (intersect(Segment(square[0][i], square[0][i - 1]), Segment(square[1][j], square[1][j - 1])) ||
                point_in_polygon(square[0][i], square[1]) != -1 || point_in_polygon(square[1][j], square[0]) != -1) {
                cout << "YES\n";
                return 0;
            }
        }
