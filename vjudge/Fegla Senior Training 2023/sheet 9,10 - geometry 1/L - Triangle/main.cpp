...
int32_t main() {
    ...
    vector<pt> possible[2];

    for (int i = 0; i < 2; i++)
        for (int B = 1; B < side[i]; B++) {
            int x = side[i] * side[i] - B * B;
            int sq = sqrtl(x);
            while (sq * sq > x) sq--;
            while (sq * sq < x) sq++;
            if (sq * sq == x) {
                possible[i].push_back(pt(B * (i == 0 ? 1 : -1), sq));
            }
        }
    ...
    for (int i = 0; i < possible[0].size(); i++) {
        for (int j = 0; j < possible[1].size(); j++) {
            if (possible[0][i].dot(possible[1][j]) == 0 && possible[0][i].y != possible[1][j].y) {
                cout << "YES\n";
                cout << "0 0\n";
                cout << possible[0][i].x << ' ' << possible[0][i].y << '\n';
                cout << possible[1][j].x << ' ' << possible[1][j].y << '\n';
                return 0;
            }
        }
    }
