...
Z f(int x, int y) {
    if (x % 2 != y % 2 || y > x) return 0;
    int Y = (x - y) / 2;
    int X = y + (x - y) / 2;
    return ncr(X + Y, X) - ncr(X + Y, X + 1);
}
...
int32_t main() {
    ...
    for (int y = max(ymin, (s + e - w) / 2); y <= min(s, e); y++) {
        for (int x = s - y; x <= w - (e - y); x++) {
            if (y == s) {
                Z v = f(x - 2, 0) * f(w - x, e - y);
                ans += v;
            } else {
                Z v = f(x - 1, s - y - 1) * f(w - x, e - y);
                debug(x, y, v);
                ans += v;
           }
        }
