...
template <typename T> struct segtree {
    ...
    void build() {
        for (int i = n - 1; i > 0; --i)
            tree[i].value = operation(tree[i << 1].value, tree[i << 1 | 1].value);
    }
...
int32_t main() {
    ...
    for (int i = 0; i < n; i++){
        cin >> arr[i], ps[i] = arr[i];
        ps[i] += i > 0 ? ps[i - 1] : 0;
        sg.tree[sg.n + i].value = ps[i];
    }
    ...
    while (q--) {
        ...
        if (t == 1) {
            int d = b - arr[a - 1];
            arr[a - 1] = b;
            sg.relative_update(a - 1, n - 1, d);
        } else {
            cout << max(sg.query(a - 1, b - 1) - (a > 1 ? sg.query(a - 2, a - 2) : 0), 0LL)<< endl;
        }
    }
